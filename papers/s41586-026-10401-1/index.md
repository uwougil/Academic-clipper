...
In the following, we use the orbital magnetization $\mathbf{M}_{o}$, the spin magnetization $\mathbf{M}_{s}$ and noncollinear antiferromagnetic Mn$_{3}$Sn (Fig. 3b) as examples to demonstrate how to analyse the SOC-induced physical properties by SOC tensor **χ**. By definition, $\mathbf{M}_{o}$ and $\mathbf{M}_{s}$ are the sums of the expectation values of orbital angular momentum operator $\hat{{\mathcal{L}}}$ and spin operator $\hat{{\boldsymbol{\sigma }}}$ over the entire Brillouin zone, respectively, expressed as:
...
in which $\mathbf{ω}^{(n)}$ is a ($2n$ + 1)th-order undetermined tensor. Equation (8) is constrained by the OSSG symmetry and is valid both for $\mathbf{M}_{o}$ and $\mathbf{M}_{s}$, but in each case, different transformation properties have to be considered for the tensors $\mathbf{ω}^{(n)}$ owing to the restriction:
...
in which $M_{s,i}$ and $M_{o,i}$ represent the *i*th components of $\mathbf{M}_{s}$ and $\mathbf{M}_{o}$ in the mentioned basis, respectively. Both $\mathbf{M}_{s}$ and $\mathbf{M}_{o}$ vectors therefore lie along the OSSG [010] direction, as expected from the corresponding MSG. Note, however, that equations (11) and (12) have been obtained by applying the symmetry conditions of the OSSG, with no explicit use of the MSG.
